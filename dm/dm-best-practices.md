...
次の表は、さまざまなシナリオにおける DM-masterと DM-workerの推奨される展開計画を示しています。
...
| <li>大規模なデータセット（1 TiB 以上）と MySQL シャードの移行およびマージ</li><li>一度限りのデータ移行</li> | 長時間のデータ移行中に DM クラスターの可用性を確保するために、3つの DM-masterノードを展開することをお勧めします。               | データソースまたは移行タスクの数に応じて、DM-workerノードをデプロイ。稼働中のDM-workerノードに加えて、アイドル状態のDM-workerノードを1～3台展開することをお勧めします。 |
